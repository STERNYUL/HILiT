...
| **v1.3** | **2026-08-30** | **증분 축약 개정.** v1.1~v1.2 증분 19건 중 **17건이 팀 문서에 이미 존재**함을 확인하고 요구사항에서 내렸다. 대신 **운영 시나리오 38건과 설계 결정 3건에 테스트 케이스 ID를 부여**해 추적성 매트릭스에 진입시켰다(§5.2·§5.3). **요구사항 총계 60 → 43건** | REF-04 §11 |
...
**REQ-FUNC-001~026 · REQ-NF-001~015는 REF-04(팀 SRS v1.0)의 식별자와 의미를 보존한다.** 증분은 **REQ-FUNC-027 · REQ-NF-016 두 건뿐**이며, 나머지는 §5.2 운영 시나리오와 §5.3 설계 결정으로 옮겼다 — **총 43건**(REQ-FUNC 27 · REQ-NF 16).
...
| **REQ-FUNC-027** | 재식별 저신뢰 후보 처리 | REF-02 AF-4 · RISK-02<br>*(v1.3 승격 — SC·REQ 어디에도 없던 유일한 처리 방침)* | Must Have | Functional | 1) 임계 미만 구간 처리 테스트<br>2) 표시 일관성 검증<br>3) QA 검증 | 재식별 신뢰도가 임계 미만인 후보는 **제외하거나 저신뢰로 표시**해야 한다. **타인의 장면을 아무 표시 없이 후보에 섞어서는 안 된다.** 임계는 REQ-FUNC-002의 `오인식 ≤ 2%`를 기준으로 한다 | Proposed | AI 리드 |
...
| **REQ-NF-016** | 미성년자 이용 정책 | REF-02 5.4<br>*(v1.3 승격 — 팀 문서 전무)* | Must Have | Security | 법무 산출물 체크리스트 승인 | 최소 연령 · 연령 확인 방식 · 보호자 동의 경로 **3종이 전부 승인되기 전까지 가입 플로우를 배포할 수 없다** `[TBD]` | Proposed | 법무 담당자 |
...
>
> **승격한 2건의 공통점** — 팀 SRS의 §4(요구사항)·§11(시나리오)·API 설명 **어디에도 없던 것**이다. REQ-FUNC-027은 오인식률 임계는 있으나 **임계를 넘은 후보를 어떻게 처리할지**가 없었고, REQ-NF-016은 전무했다.
...
> **L 4건을 일반 스프린트에 배치해서는 안 된다.** 완료 판정이 "구현됐다"가 아니라 **"85%에 도달했다"(REQ-FUNC-003) · "80%가 잘 잡혔다고 답했다"(SC-3.1)** 이기 때문이다. 정확도는 스프린트로 도달하는 것이 아니라 반복으로 **수렴**하므로, 일반 스프린트에 넣으면 **매 스프린트 미완으로 이월된다.**
>
...
| 미성년자 보호 | 최소 연령 · 연령 확인 · 보호자 동의 3종 승인 | REQ-NF-016 | 🔴 `[TBD]` 미수립 |
...
| — | **REQ-NF-016** | 정책 게이트 *(책임자: 법무 담당자)* | ReleaseGateChecklist | **TC-NF-016** |
...
| --- | --- | --- | --- |
| SC-1.1 | 정답 구간 대비 탐지 비율 ≥ 85% (Gate A) | REQ-FUNC-003 | **TC-SC-1.1** |
...
| **SC-1.F5** 🆕 | **재식별 신뢰도 미달 — 후보 제외 또는 저신뢰 표시** | **REQ-FUNC-027** | **TC-SC-1.F5** |
...
**총 38건** — 팀 v1.0의 33건 + **v1.3 신설 5건**(🆕). 신설 5건의 원천과 승격하지 않은 이유는 §11에 있다.
...
| **O9** 등장 구간 탐지율 | 정답셋 대비 재현율 | 해당 없음 | 85% 이상 | REQ-FUNC-003 · SC-1.1 | `[HYPOTHESIS]` |
...
| **1a** | **REQ-FUNC-027** 저신뢰 처리 방침 정의 | 재식별·탐지 분리 계측 지표 합의 | 없음 — 0순위와 **병렬** |
...
| --- | --- | --- | --- |
| **SC-1.F4** | Given 업로드·탐지·렌더 중 앱이 강제 종료됐을 때 / When 사용자가 재진입하면 / Then **마지막 완료 단계에서 재개**된다 | 단계 복원 성공률 `[TBD]` · 처음부터 재시작 0건 · 중간 산출물 유실 0건 | REQ-FUNC-001 · REQ-NF-008 |
| **SC-1.F5** | Given 재식별 신뢰도가 임계 미만인 구간이 있을 때 / When 후보를 제시하면 / Then **제외하거나 저신뢰로 표시**한다 | 무표시 혼입 **0건** · 임계는 `오인식 ≤ 2%` 기준 | **REQ-FUNC-027** |
