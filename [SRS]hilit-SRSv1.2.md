...
| **v1.2** | **2026-08-30** | **프로토타입 v0.6(REF-13) 대조 개정.** ① **ADR-4를 보류에서 확정으로 정정** — v1.1의 판정 오류 ② 음악 라이브러리 **5갈래 500곡** 확정 반영 ③ REQ-FUNC-036~038 신설 ④ 그룹 확정 4건 · 공유 범위 경계 · 갤러리 직접 사용 반영 | REF-13 |
...
>
> v1.1이 근거로 든 "빈 피드 양방향 리스크"는 **우려로서는 유효하나 결정을 뒤집을 사유가 아니다.** 완화 수단이 이미 **REQ-FUNC-034(빈 피드 대체 노출)** 로 존재한다. v1.2는 ADR-4를 **확정**으로 되돌리고 리스크는 RISK-05로만 추적한다.
...
- **감수하는 것** : **공개 콘텐츠가 늘지 않아 피드에 볼 것이 부족해질 수 있다.** 완화 수단은 REQ-FUNC-034(빈 피드 대체 노출)이며, 잔여 위험은 RISK-05로 추적한다
...
**REQ-FUNC-001~026은 REF-04(팀 SRS v1.0)의 식별자와 의미를 보존한다.** REQ-FUNC-027 이후가 v1.1~v1.2 증분이다 — **총 60건**(REQ-FUNC 38 · REQ-NF 22).
...
| **REQ-FUNC-036** | 마이페이지 — 소유자 뷰와 타인 뷰 | REF-13 화면 12 | Must Have | Functional | 1) 동일 계정 이중 뷰 대조 테스트<br>2) **비공개 기록 존재 은닉 검증**<br>3) QA 검증 | 같은 프로필이라도 **소유자에게는 전체공개·그룹·나만 보기 기록이 모두** 보이고, **타인에게는 전체공개 기록만** 보여야 한다. 비공개·그룹 기록은 흐리게 처리하는 것이 아니라 **목록에서 완전히 제외**되어 **타인이 그런 기록의 존재 여부조차 알 수 없어야 한다.** 카운트 표기는 `기록 / 팔로워 / 팔로잉`이며, 타인에게 노출되는 기록 수는 **전체공개 건수와 정확히 일치**해야 한다 | Proposed | 클라이언트 개발자 |
| **REQ-FUNC-037** | 기록 필터 — 종목별 · 공개 범위별 | REF-13 화면 12 | Should Have | Functional | 1) 필터 조합 테스트<br>2) 결과 정합성 검증<br>3) QA 검증 | 마이페이지에서 **종목별** 및 **공개 범위별**로 기록을 걸러 볼 수 있어야 한다 | Proposed | 클라이언트 개발자 |
| **REQ-FUNC-038** | 구도 보정 결과 확인 및 재선택 | REF-13 화면 09 | Must Have | Functional | 1) 보정 전후 비교 렌더 테스트<br>2) 재선택 복귀 테스트<br>3) QA 검증 | 구도 보정 결과를 **원본과 나란히 비교**해 확인할 수 있어야 하며, **다시 고르기**로 장면 선택 단계에 복귀할 수 있어야 한다. 복귀 시 기존 선택 상태가 보존되어야 하고, 이 동작은 `reselection_started`로 계측되어야 한다 — **ADR-3의 검증 지표다** | Proposed | 클라이언트 개발자 |
...
| US-2 | **REQ-FUNC-036** | Record Store Service · Client Shell | ProfileViewResolver | **TC-FUNC-036** |
| US-2 | **REQ-FUNC-037** | Record Store Service | RecordFilter | **TC-FUNC-037** |
| US-3 | **REQ-FUNC-038** | Client Shell · Highlight Composer | ReframePreview | **TC-FUNC-038** |
