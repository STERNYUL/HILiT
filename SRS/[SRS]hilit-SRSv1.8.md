...
- **감수하는 것 4** 🆕 : 🔴 **REQ-FUNC-023(선택 데이터 학습)의 입력이 유료 사용자로 줄어든다.** RISK-03(D1 복제)의 완화책이 *"속도가 방어다"* 인데, 학습 데이터 축적 속도가 **무료 사용자 수가 아니라 유료 사용자 수에 비례**하게 됐다. **이 개정이 감수하는 가장 큰 손실**이며, 되돌림 조건은 아래 재검토 조건 ②다
