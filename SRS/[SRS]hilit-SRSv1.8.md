...
| **v1.7** | **2026-08-30** | 🔴 **팀 SRS v1.0 전체 목차 대조 — 누락 6개 절 반영.** ① §2.2~2.4 **사용자 특성**(세그먼트 Q1~Q4 · 프로필 6유형 · **여정과 이탈 지점 도식**) ② **그림 4-1 유스케이스 다이어그램** ③ §6.3 **비즈니스 규칙 요약 10건** ④ §7 **시스템 모드 및 상태**(공개범위 상태전이도 · 편집 파이프라인) ⑤ §8 **향후 개선 사항** ⑥ §9 **다이어그램 색인**. REQ-FUNC-013에 **초대 링크 7일 · 롤백 · 회수 개수 경고** 보강. 🔴 **`UC-0n` ID 충돌 해소** — 유스케이스 번호와 사용자 유형이 같은 ID를 쓰고 있었다 | `hilit-srs-v1_0.html` §2·4·6·7·9·12 |
| **v1.10** | **2026-09-07** | 🔴 **ADR-9 신설 — 학습 라벨 회수 경계.** ADR-8 v1.9의 「감수하는 것 4」(*"선택 데이터는 유료 등급에서만 생긴다"*)를 재검토해 **수동 컷(F25)을 제3의 라벨 원천으로 회수**한다. 🔴 **두 라벨은 종류가 다르다** — 유료는 **순위**(AI 후보 중 무엇이 맞았나), 무료는 **탐지**(어디가 하이라이트인가)이며 **순위 라벨은 AI 출력에 종속되어 Gate A가 놓친 구간을 담지 못한다.** 회수를 3단으로 제한 — ① 라벨(구간·편집 이력) 무기한 ② 학습용 원본은 **동의자 한정 opt-in** ③ 🔴 **관절·생체 파생 시퀀스 저장 금지**(DD-5 존속 · 무료 GPU 초 0 유지). 「감수하는 것 4」 개정 · §5.3 TC-ADR-09 · Q24 신설 | **T16** · 사용자 결정 |
| **v1.9** | **2026-09-01** | 🔴 **ADR-8 전면 개정 — 게이트 판정 대상을 등급별로 분리.** 종전 결정(*"기능은 전부 열고 분량·횟수·규격에만 과금"*)을 **AI 추론 기능 유료 잠금**으로 뒤집고, 그 결과 무너진 하한을 다시 세웠다. 🔴 **Gate A 판정 대상 = 구독 등급**(무료 등급은 탐지를 실행하지 않아 잴 대상이 없다) · 🔴 **Gate B 판정 대상 = 무료 등급**(기록 저장·공개 범위는 전 등급 무료라 판정이 성립하고, 결제 의사와 섞이지 않는다). 금지 5건 중 **4건 존속 · 기능 잠금 금지 1건 폐기**. §1.3 용어 · REQ-FUNC-003 · §5.3 검증 기준 동반 개정 | **REF-03 PRD v2.0 ADR-13** · 사용자 결정 |
| **v1.8** | **2026-08-30** | 🔴 **죽은 참조 정리 및 유실 요구사항 복원.** ① §6.1이 **v1.3 축약 때 사라진 ID 5개**(028·029·030·031·034)를 축약 표기(`· 028`)로 참조하고 있어 검색에 안 잡혔다 — 전부 현행 SC로 교정 ② 🔴 **REQ-NF-019(정보주체 삭제 요구) 신설** — v1.1 통합 시 유실됐고 이후 참조 3곳이 REQ-NF-014(모니터링)를 잘못 가리키고 있었다. 개인정보보호법상 필수 요건이다 | 태스크 추출 대조 |
...
> ### 🔴 v1.7 — 팀 SRS 대조에서 드러난 누락
>
> v1.1~v1.6은 팀 SRS에서 **요구사항 ID(§4·§5·§11)만** 가져오고 나머지 절은 키워드 검색으로 훑었다. **그 결과 §9 사용자 특성 · §12 시스템 모드 · §7 향후 개선 · §6.3 비즈니스 규칙 · 그림 4-1이 시야에 들어오지 않았고**, §2를 팀 SRS가 아니라 PRD에서 만들었다.
>
> **v1.7은 팀 SRS 13개 절 전체를 목차 단위로 대조**해 누락 6건을 반영했다. 이제 팀 SRS의 모든 절이 이 문서에 대응을 갖는다.
>
> | 팀 SRS | 이 문서 |
> | --- | --- |
> | 01 서론 | §1 |
> | 02 이해관계자 | §2.1 |
> | 03 시스템 맥락 + 그림 3-1 | §3 |
> | 04 구체적 요구사항 + **그림 4-1** | §4 |
> | 05 추적성 매트릭스 | §5 (SC·ADR 매트릭스로 확장) |
> | 06 부록 (**6.3 비즈니스 규칙** 포함) | §6 |
> | **07 향후 개선 사항** | **§8** |
> | 08 제품 목표·성공 지표 | §5.4 · §6.6 |
> | **09 사용자 특성** | **§2.2~2.4** |
> | 10 검증 | §6.5 |
> | 11 운영 시나리오 | §5.2 · §6.7 |
> | **12 시스템 모드 및 상태** | **§7** |
> | 13 가정·의존성·제약 | §1.5 · §1.6 |
...
| **Gate A** | 등장 구간 탐지율이 임계에 도달하는지 판정하는 최우선 관문. 실패 시 이후가 무의미하다. 🔴 **판정 대상은 구독 등급**(ADR-8) |
| **Gate B** | 개인 기록 공간이 부계정을 대체할 만큼 편한지 판정하는 관문. 🔴 **판정 대상은 무료 등급**(ADR-8) |
...
| **REF-15** 🆕 | `KakaoTalk_20260831_150742960.png` — Gemini 영상 이해 검증 대화 (피겨 1분 30초) | **ADR-7의 근거.** 자연어 기준 다축 구간 추출·타임코드 산출 확인. 🔴 **인물 특정은 영상 자막 의존이었고 영상 생성은 거부됨** |
...
| **ADR-6** 🆕 | 결과물 제시 순서 | ✅ **확정 — 완성본 선제안 + 직접 고르기 병존** |
| **ADR-7** 🆕 | 후보 산출 기준의 지정 주체 | ✅ **확정 — 사용자가 자연어로 입력** |
| **ADR-8** ✏️ **v1.9** | **과금 경계 · 게이트 판정 대상** | ✅ **확정 — 무료는 사람이 손으로 · 유료는 AI가 대신 판단** · 🔴 **Gate A = 구독 등급 · Gate B = 무료 등급** |
| **ADR-9** 🆕 **v1.10** | **학습 라벨 회수 경계** | ✅ **확정 — 무료 수동 컷을 회수하되 추론 0 · 동의 한정 · 🔴 생체 파생 금지** |

> 🔺 **번호 체계 주의** — 이 문서의 `ADR-1`~`ADR-8`과 REF-03(PRD) 7-3의 `ADR-01`~`ADR-20`은 **독립된 번호 체계**다. 예컨대 이 문서 `ADR-8`(과금 경계)과 PRD `ADR-08`(후보 30개)은 **다른 결정**이다. 인용할 때 문서를 함께 적는다.
>
> 🔺 **PRD `ADR-01`(*"편집으로 과금하지 않는다"*)은 PRD v2.0에서 폐기되고 `ADR-13`으로 대체됐다.** 이 문서 `ADR-8` v1.9가 그 대체분과 정합한다.

**ADR-6 · 완성본을 먼저 보여주고, 직접 고르기 경로를 함께 둔다** → ADR-6

- **맥락** : *"AI 러닝 → 결과물 제시 → 고객 확인"* 안이 제기됐다. 캡컷 대비 조작 시간을 줄인다
- **선택지** : ① 후보 목록 우선(종전) ② **완성본 선제안 + 직접 고르기 병존** ③ 완성본만 제시
- **결정** : **② 병존.** 완성본을 재생하고 **[이대로 저장]** 과 **[직접 고르기]** 를 나란히 둔다
- **근거** : 둘 다 명시적 조작이므로 REQ-FUNC-005(*"명시적 선택 없이 확정 불가"*)를 충족하고, **후보 목록이 살아 있어 *"100% 전부에 선택·제외 가능"* 도 유지**된다. 대부분은 첫 화면에서 끝나 §3 ④ 편집 이탈에 덜 노출된다
- 🔴 **③을 기각한 이유** : 마음에 안 들 때 사용자가 할 수 있는 일이 없다. **REQ-FUNC-002의 오인식 ≤ 2%가 그대로 완성본으로 나간다**
- **감수하는 것 1** : 🔴 **첫 화면까지의 대기가 길어진다.** ①은 탐지만 끝나면 목록이 뜨지만(p95 8분), ②는 **리프레이밍·렌더까지 마쳐야** 완성본이 뜬다. **조작 시간은 줄고 대기 시간은 는다**
- **감수하는 것 2** : 🔴 **버려지는 렌더가 생긴다.** [직접 고르기]로 다른 구간을 고르면 선제안 렌더가 폐기된다 — ADR-3(*"선택분만 고화질"*)의 전제가 부분적으로 깨진다
- **감수하는 것 3** : 🔴 **품질에 종속된다.** SC-3.1의 목표가 *"잘 잡혔다 ≥ 80%"* 이므로 **5명 중 1명은 첫 화면에서 실망한다.** ①에서는 후보 목록이라 미완성임이 드러나지만 ②는 **완성품의 얼굴로 실패를 보여준다**
- **파생 요구** : ① 완성본 화면에 **AI가 무엇을 골랐는지** 표시한다 ② [직접 고르기] 진입 시 **AI 선택분이 체크된 상태**로 들어간다(FR-014 예외) ③ FR-009의 임계 `τ` 를 더 보수적으로 잡는다
- **재검토 조건** : **Gate A 미달** 또는 **SC-3.1 80% 미달** 시 ①로 되돌린다

**ADR-7 · 후보 산출 기준은 사용자가 자연어로 입력한다** → ADR-7

- **맥락** : Gemini 영상 이해로 *"기술별"* · *"의상 전환 구간"* 처럼 **자연어 기준의 다축 구간 추출**이 가능함을 확인했다(REF-15)
- **선택지** : ① 시스템이 취향을 추론 ② **사용자가 기준을 입력** ③ 도입 안 함
- **결정** : **② 사용자 입력.**
- **근거** : ①은 **D2를 없앤다** — ADR-6으로 이미 "고르기"가 "확인"으로 옮겨간 상태에서 기준까지 시스템이 정하면 사용자에게 남는 행위가 확인뿐이다. **②는 선택의 형태가 바뀐 것이지 선택이 사라진 것이 아니다.** 그리고 ①은 REQ-FUNC-023(향후 개선)에 이미 있으며 *"학습할 선택 데이터가 사용자 없이는 생기지 않아 착수 자체가 불가능"* 하다
- 🔴 **인물 식별에는 쓰지 않는다** : REF-15에서 Gemini가 인물을 특정한 근거는 **영상의 자막**이었다(*"배경 화면과 상단 자막에 출연진 이름이 나타납니다"*). **아마추어 농구 원본에는 자막이 없다.** *"내가 나온 구간"* 은 추적 API(D1), *"무엇을 했는지"* 는 Gemini — 역할을 섞지 않는다
- **감수하는 것** : 🔺 기준을 바꿀 때마다 재분석이 필요하다 — REQ-NF-013 ② 영상이해 비용이 **호출 횟수에 비례**한다
- **선행 조건** : 🔴 **SP-2** — 자막 없는 40분 농구 영상에서의 구간 정확도가 확인되기 전에는 구현하지 않는다

**ADR-8 · 무료는 사람이 손으로, 유료는 AI가 대신 판단한다 — 게이트는 등급을 나눠 판정한다** → ADR-8

> 🔴 **v1.9 개정.** 종전 결정(*"기능은 전부 열고 분량·횟수·규격에만 과금"* · **②**)을 **③(AI 기능 유료 잠금)으로 뒤집는다.** 근거는 **REF-03 PRD v2.0 §4-4의 요금제 확정**(사용자 결정 2026-09-01)이다. 아래 §"무엇이 바뀌었나"에 종전 판단 중 **살아남은 것과 폐기한 것**을 분리해 적었다.

- **맥락** : v1.8까지 이 결정은 *"편집 **기능**을 잠그지 않는다"* 였고, 그 전제 위에 🔴 **"Gate A · Gate B의 판정 대상은 무료 등급"** 이라는 하한이 서 있었다. **REF-03 PRD v2.0이 요금제를 확정하면서 그 전제가 사라졌다** — AI 트래킹·프롬프트 컷·AI 음악이 구독 전용이 되어 **무료 등급은 탐지를 실행하지 않는다.** 잴 대상이 없는 등급으로 Gate A를 판정할 수는 없다. 이 개정은 **그 하한을 다시 세우는 것**이 본체다
- **선택지** : ① 종전 유지 — 기능 전부 개방 · 분량·횟수·규격만 과금 ② 게이트 판정 대상만 바꾸고 나머지는 유지 ③ **AI 추론 기능을 유료로 두고, 게이트를 등급별로 나눠 판정**
- **결정** : **③**
- **근거** :
  - ① **①은 이미 사실과 다르다.** REF-03 v2.0 §4-5가 F2a·F2b·F5a·F27·F28을 무료에서 잠갔다. 문서가 현실을 부정하면 이행 판정이 아니라 문서가 무효가 된다
  - ② 🔴 **②는 내부 모순이다.** *"기능은 전부 열려 있다"* 와 *"무료 등급으로는 Gate A를 못 잰다"* 는 동시에 참일 수 없다. 판정 대상만 바꾸면 **왜 못 재는지**가 문서에 남지 않아 다음 사람이 되돌린다
  - ③ **값을 받는 대상을 "편집"이 아니라 "사람 대신 판단하는 추론"으로 좁힌다.** 이 경계가 **REQ-NF-013의 편당 처리 원가선과 정확히 겹친다** — 무료 등급은 GPU 0초·외부 추론 0호출이므로 원가를 만들지 않는다
  - ④ 🔴 **§2.3 Q4(약 450만 명 · 지불의향 없음)의 유입 경로는 닫히지 않는다.** 종전 근거 ④(*"기능 잠금은 사용자 수를 깎아 REQ-FUNC-023의 학습 데이터가 생기지 않게 한다"*)가 이 개정의 최대 반론인데, **무료 등급에 완주 경로를 남겨 대응한다** — 무료 사용자도 업로드 → 수동 편집 → 렌더 → **저장·공개까지 완주**한다. 다만 🔺 **선택 데이터(REQ-FUNC-023의 학습 입력)는 유료 등급에서만 생긴다** — 이것이 이 개정이 감수하는 실질 손실이다(감수하는 것 4)
  - ⑤ 🔺 **종전 근거 ⑤는 뒤집혔다.** v1.8은 *"수작업 도구를 유료로 붙이면 가장 비싼 코호트에게 이탈 지점을 파는 것"* 이라고 했다. v2.0은 수작업 도구를 **무료 등급에 넣는다** — 이탈 지점을 파는 대상이 유료에서 무료로 옮겨갔을 뿐 사라지지 않았다. **REF-03 v2.0 E12가 그 판정 실험**이다(감수하는 것 5)
- **등급 정의** (REF-03 PRD v2.0 §4-4 · §4-5) :

| 축 | 무료 | 구독 · 충전 | 원가 근거 (REQ-NF-013) |
| --- | --- | --- | --- |
| **판단 주체** | 🔴 **사람** — 좌우반전 · 수동 컷 · 수동 트래킹 · 자막 | 🔴 **AI** — 탐지·재식별·리프레이밍 · 프롬프트 컷 · AI 음악 | ① 추론 — **GPU 초 · 토큰 · 생성 단가** |
| **처리 분량** | 업로드 **2회** · 회당 ≤ 20분 · 🔴 갱신 주기 `[TBD]` | 구독 **3편/월** · 충전 **3편** · 편당 ≤ 20분 | ① 추론 — 원본 분에 비례 |
| **재작업** | 렌더 직후 원본 파기 | **원본 임시 보관 7일** — 기간 내 재편집 무차감 | ③ 저장·전송 — 원본·중간 산출물 보관 |
| **출력 규격** | 🔴 **REQ-FUNC-006 동일**(1080×1920) | 🔴 **동일** | — **등급화 금지**(아래) |

- 🔴 **금지 4건 — 유료 등급의 근거로 쓸 수 없다** *(v1.8의 5건 중 4건 존속 · 1건 폐기)* :

| 금지 | 깨지는 것 |
| --- | --- |
| 탐지·재식별·리프레이밍의 **품질 등급화** | 🔴 **Gate A 판정이 무의미해진다.** 구독과 충전이 다른 모델·다른 파라미터를 쓰면 어느 쪽으로 잰 85%인지가 갈린다. **유료 등급 내부에서 다시 나누지 않는다** |
| 후보 **개수** 등급화 | **Q7**(후보 약 30개 · 가정 A3)을 실측으로 확정할 수 없게 된다. 유료 전 등급 **30개 동일** |
| **`[직접 고르기]` 접근 제한** | REQ-FUNC-005 · ADR-6 · 차별점 **D2** — 병존 경로가 요금제에 종속되면 D2가 이름만 남는다. 유료 전 등급에서 **항상 노출** |
| 신뢰도 임계 **`τ`** 의 등급별 차등 | REQ-FUNC-027 — *"확신이 없으면 보여주지 않는다"* 가 요금제에 종속된다. **단일 값** |
| **기록 저장 · 공개 범위** | REQ-FUNC-009 · 010 · ADR-4 · **Gate B** — 🔴 **강화한다.** 전 등급 무료이며 **구독 만료·충전 소멸 후에도 `Record` 접근을 막지 않는다**(REF-03 v2.0 AC4-4 · AC8-F1) |
| ~~기능 잠금 금지~~ | ⚠️ **폐기.** AI 추론 기능 5종(F2a·F2b·F5a·F27·F28)을 무료 등급에서 잠근다 — 이것이 이 개정의 본체다 |

- 🔴 **하한 — 게이트는 등급을 나눠 판정한다** *(v1.8의 "판정 대상은 무료 등급"을 대체한다)* :

| 게이트 | 판정 지표 | 🔴 판정 대상 등급 | 근거 |
| --- | --- | --- | --- |
| **Gate A** | REQ-FUNC-003 **제외 후 탐지율 ≥ 85%** · REQ-FUNC-006(1080×1920 · 4점 이상 ≥ 80%) · REQ-FUNC-027(`τ`) | 🔴 **구독 등급** | AI 탐지가 구독 전용이 되어 **무료 등급에는 잴 대상 자체가 없다.** 구독과 충전은 같은 모델·같은 `τ`를 쓰므로 **어느 쪽으로 재도 같은 값**이어야 한다 — 다르면 위 금지 1번이 코드에 들어간 것이다 |
| **Gate B** | 기록 공간 수용성 — 부계정을 대체할 만큼 편한가 | 🔴 **무료 등급** | 기록 저장·공개 범위는 **전 등급 무료**로 남았으므로 판정이 성립한다. 🔴 **무료 등급으로 재야 결제 의사와 섞이지 않는다** — 유료 사용자는 이미 돈을 냈으므로 *"편한가"* 에 편향이 들어간다 |

- 🔴 **판정 대상이 아닌 등급으로 게이트를 판정한 건이 0건**이어야 한다. Gate A를 무료 등급으로, Gate B를 유료 등급으로 판정한 기록이 남으면 **그 판정은 무효**다
- **감수하는 것 1** : 🔺 **REF-03 `ADR-01`(*"편집으로 과금하지 않는다"*)과의 충돌은 해소됐다.** REF-03 PRD v2.0이 `ADR-01`을 **폐기하고 `ADR-13`으로 대체**했으므로 v1.8의 *"SRS 단독 판정"* 표기를 내린다. 🔺 다만 **CLAUDE.md 규칙 4·6의 개정이 남아 있다**
- **감수하는 것 2** : 🔺 무료 쿼터 값이 `[TBD]`인 동안 이 결정은 **이행 판정이 불가능**하다 — **Q3**(편당 처리 원가 상한)이 유일한 입력이며, REF-03 v2.0이 **편당 매출 상한 3,300원**(구독 9,900원 ÷ 3편)을 확정해 **Q3의 판정 기준선이 생겼다**
- **감수하는 것 3** : 🔴 우선 처리 큐를 도입할 경우 *"무료를 일부러 느리게 만들지 않는다"* 를 함께 지켜야 한다. **REQ-NF-003(탐지 p95 ≤ 8분)은 구독·충전에 동일 적용**되며 차이는 **혼잡 구간에서만** 발생한다. 🔴 **구독과 충전 사이에 우선 처리 차등을 두지 않는다**
- **감수하는 것 4** ✏️ **v1.10 — 축소됐다** : 🔴 **REQ-FUNC-023의 입력 중 「순위 라벨」만 유료 사용자로 줄어든다.** 종전 문면(*"학습 입력이 유료 사용자로 줄어든다 — 이 개정이 감수하는 가장 큰 손실"*)은 **수동 컷(F25)이라는 제3의 라벨 원천을 계산에 넣지 않았다.** 🔴 **ADR-9로 「탐지 라벨」은 무료 등급에서 회수한다** — 오히려 **AI 후보에 없어 유료 라벨로는 영원히 학습되지 않는 Gate A 미달분**을 담는 유일한 입력이다. RISK-03의 *"속도가 방어다"* 도 **무료 사용자 수에 부분적으로 다시 연결된다.** 🔺 **손실이 사라진 것은 아니다** — 순위 라벨은 여전히 유료 종속이며 **되돌림 조건 ②는 그대로 존속한다**
- **감수하는 것 5** 🆕 : 🔺 **§2.4 ④ 편집 이탈이 무료 등급에서 되살아난다.** *"30분 초과 시 포기"* 가 v1.8이 수작업 도구를 막은 이유였고, v2.0은 그 도구를 무료 등급에 넣었다. **판정 장치는 REF-03 v2.0 E12**(무료 완주율 ≥ 40% · 세션 p90 ≤ 30분)이며, 실패 시 무료 등급 설계를 되돌린다
- 🔴 **페이월을 첫 완주 이전에 노출하지 않는다** : Gate B(기록 공간 수용성)가 페이월 노출로 오염되면 **Gate B 판정 자체가 무효**가 된다. **판정 대상이 무료 등급이므로 이 조항은 v1.8보다 더 중요해졌다**
- **선행 조건** : 🔴 **REF-03 `ADR-04`에 따라 요금제 도입 시점은 Gate B 통과 후다.** 지금 구현 대상은 요금제가 아니라 **계측 이음새**뿐이다 — Entitlement Resolver를 **CT-007과 같은 방식으로 전부 무제한을 반환하는 Mock**으로 두고, 실제 요금제 도입 시 **호출부 코드 변경이 0건**이어야 한다
- **재검토 조건** :
  - ① **구독 등급이 Gate A를 통과하지 못하면** 요금제 전체를 보류하고 원가 구조를 먼저 재설계한다 (ADR-2 · ADR-3 재검토)
  - ② 🆕 **무료 등급이 Gate B를 통과하지 못하거나 E12가 실패하면** — 무료 등급의 판단 주체 경계를 되돌린다. 후보는 **①수작업 도구 축소 ②AI 무료 체험 1편 제공** 둘이며, ②를 택하면 **Gate A 판정 대상이 무료 등급으로 돌아온다**

##### 무엇이 바뀌었나 — v1.8 → v1.9 대조

| 항목 | v1.8 | v1.9 |
| --- | --- | --- |
| 결정 | ② 기능 전부 개방 | **③ AI 추론 기능 유료** |
| **Gate A 판정 대상** | 무료 등급 | 🔴 **구독 등급** |
| **Gate B 판정 대상** | 무료 등급 | **무료 등급** *(유지)* |
| 금지 항목 | 5건 | **4건 존속 + 1건(기능 잠금) 폐기** |
| 과금 축 | 분량 · 재작업 · 규격 | **판단 주체** + 분량 · 재작업 *(규격은 등급화 금지로 이동)* |
| REF-03 `ADR-01`과의 관계 | 🔺 충돌 · SRS 단독 판정 | ✅ **해소** — `ADR-01` 폐기 · `ADR-13`으로 대체 |
| 살아남은 것 | — | **품질 등급화 금지 · `τ` 단일값 · `[직접 고르기]` 개방 · 후보 30개 · 기록 저장 무료 · 페이월 시점 · Mock 우선** |

**ADR-9 · 무료 등급의 수동 컷을 학습 라벨로 회수한다 — 추론 0 · 동의 한정 · 생체 파생 금지** → ADR-9

- **맥락** : ADR-8 v1.9의 「감수하는 것 4」는 *"선택 데이터는 유료 등급에서만 생긴다"* 를 **이 개정이 감수하는 가장 큰 손실**로 확정하고, 되돌림 수단을 **재검토 조건 ② 하나**(①수작업 도구 축소 ②AI 무료 체험 1편)로 남겼다. 🔴 **둘 다 ADR-8의 등급 정의 자체를 훼손한다.** 그러나 그 계산에는 **수동 컷(F25)이라는 제3의 라벨 원천이 빠져 있었다**
- 🔴 **두 라벨은 종류가 다르다** — 이것이 이 결정의 전부다

| | 유료 경로 | 무료 경로 |
| --- | --- | --- |
| 사용자 행위 | AI 후보 30개 중 **선택** | 타임라인에서 **직접 구간 지정** |
| 라벨이 답하는 것 | *"AI가 낸 것 중 **무엇이 맞았나**"* — **순위** | *"**어디가** 하이라이트인가"* — **탐지** |
| AI 출력 종속 | 🔴 **있다** | 🟢 **없다** |
| Gate A 미달분(놓친 구간) | 🔴 **후보에 없어 영원히 학습되지 않는다** | 🟢 **담긴다** |

- **결정** : 무료 등급의 수동 컷을 REQ-FUNC-023의 학습 입력으로 **회수한다.** 회수 범위는 **3단으로 제한**한다

| # | 대상 | 범위 | 근거 |
| --- | --- | --- | --- |
| **①** | 🟢 **라벨** — 수동 컷 구간(positive) · 편집 이력 | **무기한** — 선택 이력(DD-8)과 동일 취급 | 사용자가 이미 하는 행위의 기록이므로 **추론 0건 · GPU 초 0** |
| **②** | 🟡 **학습용 원본** | 🔴 **동의자 한정 opt-in** — 미동의분은 **렌더 직후 파기**(현행 유지) | 🔺 **라벨만으로는 학습이 성립하지 않는다.** 동의·철회 경로 필수(REQ-NF-010 · REQ-NF-019) |
| **③** | 🔴 **관절·생체 파생 시퀀스 — 저장하지 않는다** | — | 추출이 **GPU 추론**이라 ADR-8 검증 ④(*무료 편당 GPU 초 = 0*)를 깨고, **원본보다 오래 사는 인체 데이터**라 **DS DD-5를 뒤집는다** |

- **negative 라벨의 수명 분리** : 🔺 시청 로그로 negative를 만들면 **입력은 DS §4.4의 90일, 라벨은 무기한**이라 수명이 어긋난다. **원시 시청 로그는 90일에 파기**하고, 그로부터 **파생된 negative 구간만 라벨로 승격**해 무기한 보관한다
- **ADR-7과의 경계** : 🔴 *"시스템이 취향을 추론하지 않는다"* 는 **런타임 개인화**에 대한 규칙이다. 이 결정은 **전역 모델 학습**이며, 🔴 **개인별 되먹임**(그 사용자의 과거 선택으로 그 사용자의 후보를 바꾸는 것)**은 금지로 남는다**
- **감수하는 것 1** : 🔺 **학습용 원본 보관이 새 저장 원가를 만든다.** ADR-8의 등급 축(③저장·전송)이 아니라 **데이터 확보 원가**라 별도 항목이며, REQ-NF-013에 반영이 필요하다
- **감수하는 것 2** : 🔺 **동의율이 낮으면 ②가 비어 ①이 학습에 쓰이지 못한다.** 동의율은 **Baseline: 미측정 · Target: TBD**(Q24)
- **선행 조건** : 🔴 **`FACE_CONSENT`(REQ-NF-010 산출물 4종) 승인 전에는 ②를 배포하지 않는다.** ①의 스키마 확정은 승인과 무관하게 진행한다 — **결정 게이트가 아니라 배포 게이트**다
- **재검토 조건** : 무료 등급 라벨이 **Gate A 미달분을 실제로 담고 있지 않으면**(제외 전/후 분리 집계로 판정) ①의 무기한 보관을 축소한다
...
- **감수하는 것** : 원본이 서버로 전송되므로 **얼굴 정보 처리(REQ-NF-010)·삭제 요구 이행(REQ-NF-019)의 부담이 온디바이스 대비 커진다.** GPU 원가가 사용량에 비례한다(RISK-03)
...
| **ADR-5** 외부 내보내기 | 🔴 **REF-01에 근거가 없다.** 허용하면 **종목 크리에이터**의 진입 조건이 되지만 Core Job "앱을 옮기지 않고 완성"(REQ-FUNC-008)과 정면 충돌한다. 상위 문서가 침묵하는 사안을 SRS가 단독으로 결정할 수 없다 |
...
> 근거: REF-05 §9.6.6 User characteristics

**대상 선별의 1순위 기준은 원본 영상 보유 여부다.** 앱에서 촬영하지 않으므로 **원본이 없으면 제품이 작동하지 않는다.** 2순위는 지불의향이다.

| 세그먼트 | 정의 | 규모 | 처리 |
| :--: | --- | ---: | --- |
| **Q1** | 원본 있음 · 지불의향 있음 (이미 올리는 사람) | 약 **50만 명** | **대상 · 1순위** — 촬영 습관이 있고 편집 부담을 체감. 전환 비용이 가장 낮다 |
| **Q4** | 원본 있음 · 지불의향 없음 (찍어놓고 안 올림) | 약 **450만 명** | **대상 · 2순위** — 페인이 가장 크고 시장이 **9배**. 값을 편집이 아니라 **기록 축적**에서 받기로 정했으므로 무료 확보가 사업의 본체 |
| Q2 | 원본 없음 · 지불의향 있음 | 약 200만 명 | **보류** — 촬영 가이드(REQ-FUNC-025) 단계에서 열린다 |
| Q3 | 순수 소비자 | 약 2,800만 명 | **대상 아님** |

*규모는 전부 `[HYPOTHESIS]` — 시장 산정값이며 실측이 아니다.*

> **Q4가 Q1보다 9배 크다는 사실이 이 제품의 수익 구조를 결정했다.** 페인이 가장 큰 집단에 지불의향이 없으므로 **편집 기능에 값을 매기면 그들에게 닿지 못한다.** 값을 기록 축적과 관계망에서 받기로 한 결정이 여기서 나온다.

## 2.3 대상 사용자 프로필

| 사용자 유형 | 세그 | 특성 | 주요 제약 | 사용 맥락 | Pain | 대응 요구사항 |
| --- | :--: | --- | --- | --- | :--: | --- |
| **직장인 동호회 참가자** | Q1 | 주 2회 사회인 농구 · **본계정과 종목 부계정 병행** | 원본 47편 적재 / 3개월 업로드 2건 | 경기가 끝난 밤, **40분 원본이 하나 더 늘었을 때** | P1·P2·P3 | REQ-FUNC-001~010 |
| **동아리 촬영 담당** | Q1 | 팀원 8명분 촬영을 전담 · **자기 기록은 늘 후순위** | 자기 기록 **0건** | 경기 후 팀원들이 *"내 장면 보내줘"* 라고 요청할 때 | P1 | REQ-FUNC-002~005 · 013 |
| **종목 크리에이터** | Q1 | 주 3회 업로드 목표 · 촬영 1시간 + 편집 3시간 | **외주 월 60만 원대가 수익 초과** | 업로드 주기를 지켜야 하는데 **원본만 세 개 쌓였을 때** | P1 | REQ-FUNC-003~008 · REQ-NF-018 |
| **팀 운영자** | Q1 | 팀원 12명 · **편집 인력은 본인 1명** | 매주 원본 생성 / 업로드 월 2회 | 팀원들이 *"내 장면 있어?"* 라고 묻는데 **편집자가 자기뿐일 때** | P1·P3 | REQ-FUNC-010 · 013 · 014 |
| **저장공간 압박형** | Q4 | 액션캠 50분 촬영 · 파일 4GB | 유료 편집 앱 **3개 결제 후 전부 해지** | 주말 경기 후 **저장공간 부족 알림이 뜰 때** | P1·P3 | REQ-FUNC-009 · 010 · 019 |
| **완성도 부담형** | Q4 | 갤러리 미편집 영상 약 200개 · 업로드 **연 3회** | 편집 30분 초과 시 매번 중도 포기 | 주말 영상을 정리하려 **편집 앱을 켰을 때** | P1 | REQ-FUNC-006~008 |

*공통 Pain P1~P3의 정의와 실패 판정선은 §1.1에 있다.*

> 🔴 **사용자 유형은 이름으로 참조한다.** `UC-0n` 은 **§4 그림 4-1의 유스케이스 번호**이며 사용자 유형이 아니다. v1.6까지 두 의미가 섞여 있었고 v1.7에서 분리했다.

> ### 🔴 공통 숙련도 — 장벽은 도구 사용 능력이 아니다
>
> **촬영에는 마찰이 없다**(1분 이내 완료). 문제는 **촬영 이후**에 발생하며, **편집 도구 사용 능력이 아니라 시간과 반복 노동**이 장벽이다.
>
> 이 구분이 요구사항에 직접 영향을 준다 — **튜토리얼이나 사용성 개선으로는 풀리지 않는다.** 작업량 자체를 없애야 한다.

**촬영 담당과 팀 운영자는 본 범위에서 절반만 충족된다.** 완전 충족은 팀 공유 편집(REQ-FUNC-024)이 필요하나 **팀 계정·공유 권한·얼굴 동의·인원 비례 원가 넷이 전부 미정**이라 제외했다.

### 2.3.1 이용자 클래스에 포함하지 않은 행위자

| 행위자 | 상호작용 | 대응 요구사항 |
| --- | --- | --- |
| **법정대리인** | 미성년 이용자 가입 동의 · 영상 내 미성년자 동의 · 열람·삭제·철회 | REQ-NF-016 · 017 |
| **운영자** | 신고 접수분 처리 | REQ-FUNC-016 |

---

## 2.4 사용자 여정과 이탈 지점

> **그림 2-1. 사용자 여정과 이탈 지점 (Flow Chart)**
>
> **무엇을 보여주나** — 사용자가 촬영부터 재방문까지 거치는 여섯 단계와, 그중 어디서 포기하는지다.
> **읽는 법** — 초록은 마찰이 없는 단계, 주황은 **이탈이 몰리는 단계**다. 아래로 뻗은 점선이 **포기 경로**이고, 연한 초록 상자는 그 이탈을 **막는 요구사항**이다.
...
**핵심 — 이탈은 ①촬영이 아니라 ③찾기·④편집·⑤남기기에 몰려 있다.** 촬영은 이미 습관이 됐고 **문제는 그 뒤에서 시작된다.**
...
> 🔴 **이탈은 ⑥에서 끝나지 않고 ①로 거슬러 올라간다.** 한 번이라도 편집에서 지치면 **다음 주에 삼각대를 아예 안 들고 간다.** 그때는 이 제품이 개입할 지점 자체가 사라진다.
>
> 이것이 **O1(체감 탐색 시간 5분)** 이 단순한 성능 목표가 아닌 이유다. **③이 빠르지 않으면 ①이 멈춘다.** [REF-08]

### 2.4.1 🔴 「장면 선택」은 하나의 요구가 아니다

**여정지도에서 5명이 같은 단계에서 막혔는데 이유가 전부 달랐다.** [REF-08 §7-1]

| 막히는 지점 | 필요한 것 | 대응 |
| --- | --- | :--: |
| 지하철에서 한 손으로 30개는 많다 | **개수 조정** | 🟡 가정 A3 · Q7 |
| *"잘 나온 것"* 순인데 *"처음 된 것"* 을 찾는다 | 🔴 **정렬 기준 선택** | 🔴 **요구사항 없음** |
| 후보에 다른 사람이 섞여 있다 | 신뢰도 처리 | ✅ REQ-FUNC-027 |
| 후보에 **내 장면이 아예 없다** | 저신뢰 구간 안전망 | ✅ SC-1.F2 · F6 |
| 다수 인원분 선택은 물리적으로 불가능 | 선택 자동화(B2B) | ➖ REQ-FUNC-024 (범위 밖) |

> 🔴 **「정렬 기준 선택」에 대응하는 요구사항이 없다.** REQ-FUNC-004는 *"온전히 잡힌 구간을 우선 배열"* 만 정한다. 그러나 사용자가 찾는 것이 *"잘 나온 것"* 이 아니라 ***"처음 성공한 것"*** 일 때 그 정렬은 오히려 방해가 된다. → **Q19**

### 2.4.2 🔴 지표에 잡히지 않는 이탈이 셋

| 이탈 유형 | 종료 방식 | 지표에 잡히나 |
| --- | --- | :--: |
| 진입 거부 | 설치조차 하지 않음 | ❌ 애초에 안 들어옴 |
| 실패 후 삭제 | 결과가 나쁘면 조용히 지움 | ⚠️ 실패 로그로만 |
| **무음 이탈** | 아무 피드백 없이 앱 삭제 | ❌ **아무 흔적 없음** |

> **셋 다 "쓰다가 나간 사람"이 아니라 "처음부터 못 들어온 사람"이다.** [REF-08 §7-3]
>
> REQ-NF-014(모니터링)의 알림 대상은 **발생한 실패**뿐이다. 완주율도 *"시작한 작업"* 의 완주율이라 **시작조차 안 한 경우를 담지 못한다.** → **Q20**

### 2.4.3 최고 감정은 저장 직후가 아니다

| 이용자 | 최고 감정 시점 | 언제 |
| --- | --- | --- |
| 직장인 동호회 참가자 | 다음 주 촬영 — *"이번엔 금방 끝났으니까"* | 다음 주 |
| 팀 운영자 | 재등록 상담에서 **기간 비교 자료** 활용 | 3개월 후 |
| 검증 페르소나 | 지난 기록 **다시 열어보기** | 3개월 후 |

> **뒤 둘은 같은 기능이다** — 기간 비교 뷰. **REQ-FUNC-020(기록 타임라인)** 하나가 둘을 덮는다. [REF-08 §7-2]
>
> 🔺 **REQ-FUNC-020이 Should Have인 것은 재검토 대상이다.** 세 이용자의 **최고 감정 지점**이자 O7(북극성)의 체감 경로인데 MVP 밖에 있다. → **Q21**
...
> **읽는 법** — 왼쪽 둥근 상자가 **사람(액터)**, 가운데 영역이 **시스템이 제공하는 기능**, 오른쪽은 **사람이 아닌 외부 시스템**이다. 각 유스케이스의 `REQ-FUNC-0NN` 이 §4.1 표의 번호와 그대로 대응한다.
...
flowchart LR
    ...
    subgraph SYS["HILiT 시스템 경계"]
        ...
        UC2("UC-02 내 장면 자동 탐색<br/>REQ-FUNC-002·003·027")
        ...
        UC10("UC-10 동의 관리<br/>REQ-NF-016·017")
    end
...
**주황(UC-02)은 실패하면 나머지가 무의미해지는 관문(Gate A)** 이고, **노랑(UC-05)은 이 제품의 핵심 주장인 *"올리지 않아도 남는다"* 가 성립하는 지점(Gate B)** 이다.
...
| **REQ-FUNC-003** | 사용자 등장 구간 자동 탐지 | US-1 · REF-03 4-1 (F2b) | Must Have | Functional | 1) 정답셋 회귀 배치<br>2) **IoU 기반 정탐 판정**<br>3) **제외 전/후 분리 집계**<br>4) QA 검증 | 사람이 표시한 정답 구간 대비 **사용자에게 제시된**(REQ-FUNC-027 제외 적용 후) 탐지 구간 비율이 **85% 이상**이어야 한다 `[HYPOTHESIS · Gate A 판정]`. 🔴 **판정 대상은 구독 등급**이며, 유료 등급 내부(구독·충전)는 같은 모델·같은 `τ`를 쓰므로 **어느 쪽으로 재도 같은 값**이어야 한다(ADR-8). **제외 전 비율도 함께 집계**하되 이는 진단용이며 게이트 판정에 쓰지 않는다 | Draft | AI 리드 |
...
| **REQ-FUNC-013** | 그룹 — 소그룹 공유 | US-4 · REF-03 4-1 (F23)<br>+ REF-13 화면 03 | Must Have | Functional | 1) 생성·초대·탈퇴 테스트<br>2) **3경로 노출 차단 검증**<br>3) 이름 중복 허용 검증<br>4) QA 검증 | 승인 절차 없이 생성되며 **인원 상한 20명**(생성자 포함)이고, **초과 초대는 부분 생성 없이 롤백**한다. **링크 초대는 가입·로그인한 사용자만 참여 가능**하며 **유효기간 7일**이다. **그룹 이름 중복을 허용**한다. 그룹 공개 기록은 구성원에게만 노출되며 **팔로우 관계만으로는 절대 열리지 않아야 한다**. 탈퇴 시 그 그룹에 올린 기록을 회수하되 본인 기록에는 남기고, **나가기 전 회수될 개수를 실제 값으로 경고**해야 한다 `[SOURCE]` | Draft | 백엔드 리드 |
...
| **REQ-NF-013** | 편당 처리 원가 가드레일 | REF-03 5-4 | Must Have | Cost | 처리 물량 대비 원가 추적 | 처리 물량은 **연 12만 편**(1만 명 × 월 1편)에 직접 비례한다 `[HYPOTHESIS]`. **편당 처리 원가는 네 항목의 합**으로 산정한다 — ① **추론** ② **영상 이해** ③ 🔴 **저장·전송**(원본 4GB를 추론 대상으로 전달하는 전송량이 지배 요인) ④ **렌더**(사용자 단말 수행 시 0원). **상한이 확정되기 전에는 무제한 업로드를 열지 않는다** `[TBD]` | Draft | 사업 담당자 |
...
| **REQ-NF-019** | 정보주체 삭제 요구 이행 | REF-02 5.4<br>*(v1.8 신설 — 🔴 v1.1 통합 시 유실된 것을 복원)* | Must Have | Security | 1) 삭제 요청 시나리오 테스트<br>2) 파생물 잔존 스캔<br>3) 법무 확인 | 사용자의 삭제 요청 시 **원본 · 추적 궤적 · 등장 구간 · 후보 · 완성 영상 · 기록 및 객체 스토리지의 모든 파생물을 전량 삭제**해야 한다. 🔴 **논리 삭제는 이 요구를 만족하지 않는다.** 계측 이벤트는 식별자를 **비식별화**하고 집계는 유지한다. 처리 시한은 미정이다 `[TBD]` | Proposed | 백엔드 리드 |
...
| **M** | 신규 설계가 필요하나 **결과가 예측 가능** | 한 스프린트에 1건 | REQ-FUNC-001 · 004 · 007 · 011 · 013 · 014 |
| **L** | **성능이 결과를 좌우** — 종료 조건이 *"동작한다"* 가 아니라 **임계 도달**이다 | **부적합** | REQ-FUNC-002 · 003 · 006 · **027** · 023 · 🔺 **008** |

> 🔺 **REQ-FUNC-008(합치기·렌더링)을 M에서 L로 올린다.** 렌더를 **사용자 단말에서 수행하는 구현**(SRS v2.2 §3.3)에서는 종료 조건이 *"렌더가 된다"* 가 아니라 **"어느 단말 등급까지 되는가"** 이므로, 모델 정확도와 같은 **수렴형**이다. 일반 스프린트에 두면 매번 미완으로 이월된다. 서버 렌더 구현으로 되돌아가면 M으로 환원한다.
...
| 개인정보 — 정보주체 권리 | 삭제 요청 시 원본·파생물 전량 삭제 | **REQ-NF-019** | `[PROPOSED]` |
...
> **US-6(크리에이터) 추적 복원** — v1.4까지 이 Story는 사용자 프로필(**종목 크리에이터**)로만 존재하고 **추적성 매트릭스에 행이 없었다.** Story의 핵심 요구(*"편집 시간을 3시간에서 크게 줄여 주기를"*)에 대응하는 요구사항이 **REQ-NF-018로 신설**되면서 체인이 연결됐다.
>
...
| — | **REQ-NF-019** | Record Store Service · Video Service | ErasureExecutor | **TC-NF-019** |
...
| **ADR-9** 🆕 | 학습 라벨 회수 경계 — 추론 0 · 동의 한정 · 생체 파생 금지 | ① 🔴 **무료 등급 라벨 수집 경로의 GPU 초 = 0 · 외부 추론 호출 0건**인가<br>② 🔴 **관절·생체 파생 시퀀스 테이블이 스키마에 존재하지 않는가**(DD-5 존속)<br>③ 학습용 원본이 **동의 레코드 없이 보관된 건 0건**인가<br>④ 원시 시청 로그가 **90일을 넘겨 잔존한 건 0건**인가<br>⑤ 🔴 **개인별 되먹임**(사용자 과거 선택 → 그 사용자 후보 변경) **0건**인가 | **TC-ADR-09** |
| **ADR-8** ✏️ **v1.9** | 과금 경계 — 무료는 사람이 손으로, 유료는 AI가 대신 판단 | ① 🔴 **Gate A 판정 입력이 구독 등급인가** · 🔴 **Gate B 판정 입력이 무료 등급인가** — 판정 대상이 아닌 등급으로 판정한 건 **0건**<br>② 금지 4건이 등급 분기 조건으로 코드에 존재하지 않는가 — 특히 `τ` · 후보 개수 · `[직접 고르기]` · 출력 규격<br>③ Entitlement Resolver를 Mock → 실제로 교체할 때 **호출부 변경 0건**인가<br>④ 🆕 **무료 등급 편당 GPU 초 = 0 · 외부 추론 호출 0건**인가<br>⑤ 🆕 **결제 만료·소멸 후 `Record` 접근 차단 0건**인가 | **TC-ADR-08** |
...
| --- | --- | --- | --- | --- |
| **Media Ingest** | POST | `/videos` | 원본 업로드 개시 · 코덱 검증 · 이어올리기 세션 생성 | REQ-FUNC-001 · **SC-1.F1 · F3** |
| **Media Ingest** | PATCH | `/videos/{id}/chunks` | 청크 업로드 · 진행률 갱신 | REQ-FUNC-001 · **SC-1.F3** |
| **Media Ingest** | DELETE | `/videos/{id}` | 원본 삭제 | REQ-FUNC-019 · **REQ-NF-019** |
...
| **Highlight Composer** | GET | `/videos/{id}/candidates` | 후보 목록 조회 (신뢰도 플래그 포함) | REQ-FUNC-004 · **027** · **SC-1.F2 · F6** |
| **Highlight Composer** | GET | `/music` | 음악 라이브러리 목록 | REQ-FUNC-007 |
| **Highlight Composer** | POST | `/records` | 선택 확정 → 리프레이밍 · 렌더 · 기록 저장 | REQ-FUNC-005~009 · **SC-3.F1 · F2** |
...
| **Feed** | GET | `/feed?tab=following\|recommend\|group` | 탭별 피드 조회 · 빈 피드 대체 | REQ-FUNC-014 · **SC-6.F1** |
...
| **Telemetry** | POST | `/events` | 계측 이벤트 수집 | REQ-NF-014 |

## 6.2 Entity & Data Model

> **이 절과 `DS/[DS]hilit-DSv1.1.md` §4.2의 분담** — 여기에는 **요구사항 수준의 제약**만 둔다(필드·타입·키·요구사항이 직접 정한 CHECK). **인덱스 · 보존 기간 · Cascade 세부 · Prisma 사상**은 DS §4.2에만 있다. **중복 기재하지 않는다.**
...
| **Selection** | **사용자 선택 기록.** 🔄 렌더 완료 시 `TrainingLabel`로 **승격**된다(DD-10) | REQ-FUNC-005 · 023 |
...
| **ManualCut** 🆕 | **무료 등급 수동 컷 구간.** 🔴 후보 없이 사람이 직접 그린 **탐지 라벨** | REQ-FUNC-023 · ADR-9 ① |
| **ManualCutEdit** 🆕 | 경계 미세조정 이력 | REQ-FUNC-023 · ADR-9 ① |
| **WatchSession** 🆕 | 실제 재생 구간 — negative 라벨의 원천 · **90일** | REQ-FUNC-023 · ADR-9 |
| **TrainingLabel** 🆕 | 🔴 **학습 파이프라인의 유일한 입력** · 무기한 | REQ-FUNC-023 · ADR-9 |
...
**삭제 요구 (REQ-NF-019)** — 사용자의 삭제 요청 시 원본·추적 궤적·등장 구간·후보·완성 영상·기록 및 **객체 스토리지의 모든 파생물을 전량 삭제**한다.
...
**요구사항 표에 흩어져 있는 규칙 중 제품 정의를 지탱하는 열 가지다. 하나라도 깨지면 제품 주장이 성립하지 않는다.**

| # | 규칙 | 근거 |
| :--: | --- | --- |
| **1** | **기록 우선** — 공개 범위를 정하기 전에 기록이 **이미 저장돼 있어야** 한다. 저장과 공개는 분리된 행위다 | REQ-FUNC-009 · Gate B |
| **2** | **기본값은 나만 보기** — 저장 직후 공개 범위는 `PRIVATE`. **기본이 공개면 *"올리지 않아도 남는다"* 가 성립하지 않는다** | REQ-FUNC-010 · ADR-4 |
| **3** | **최종 선택권은 사람에게** — 사용자의 명시적 선택 없이 결과물을 확정하지 않는다 | REQ-FUNC-005 · SC-3.3 |
| **4** | **공개 범위 서버 강제** — 모든 조회 경로에서 서버가 범위를 강제한다. **클라이언트 필터링만으로는 불충분하다** | REQ-NF-009 |
| **5** | **그룹 인원 상한 20명** — 생성자를 포함하며, **초과 초대는 부분 생성 없이 롤백**한다 | REQ-FUNC-013 · SC-5.F1 |
| **6** | **그룹 탈퇴 시 회수** — 나가면 그 그룹에 올린 기록도 함께 내려간다. **본인 기록에는 남으며**, 나가기 전 **회수될 개수를 실제 값으로 경고**한다 | REQ-FUNC-013 · SC-2.F1 |
| **7** | **링크 초대는 가입자 한정** — 링크는 누구에게나 보낼 수 있으나 참여는 **가입·로그인한 사용자만** 가능하다. **유효기간 7일** | REQ-FUNC-013 · SC-5.3 |
| **8** | **관계와 공개의 분리** — 팔로우 관계는 **공개 기록만** 정한다. 비공개 공유는 **그룹**이 맡는다 | REQ-FUNC-012 · SC-5.5 |
| **9** | **원본 보관 범위** — 원본은 **결과물 확보 시점까지만** 보관하는 것을 기본으로 한다 | REQ-FUNC-019 · REQ-NF-014 |
| **10** | **게이트 순서 불변** — **Gate A → Gate B** 순서를 바꿀 수 없다. Gate A 실패 시 이후 단계에 착수하지 않는다 | §6.5 |
...
| **Phase 2** 클로즈드 베타 | 초대 · 직장인 동호회 참가자 · 동아리 촬영 담당 · 팀 운영자 | + REQ-FUNC-009~011 · 032 | **Gate B** · O5 | 저장 성공률 ≥ 99.9% · O5 `[TBD]` | 기록 구조 재설계 |
...
| **Q5** | 외부 내보내기를 허용할 것인가 | 종목 크리에이터 진입 여부 · ADR-5 | 제품 | Phase 3 전 |
| **Q6** | 공개 범위 기본값 (ADR-4) | O5 · 첫 저장 이탈 | 제품 · 디자인 | Gate B |
| **Q7** | 후보 약 30개가 적정한가 (가정 A3) | 사용자 피로 | 제품 | Phase 1 실측 후 |
| **Q8** | 팀 공유 편집을 어떻게 교착에서 꺼낼 것인가 | 동아리 촬영 담당 · 팀 운영자가 절반만 충족 | 제품 | `[TBD]` |
| **Q9** | 음원 라이선스 조달 방식 (증빙 3종) — **자체 제작 / 구매 / 제휴 / 🟢 무료 라이브러리** 4갈래. 경로별 비교는 `tasks/NF-016` §조달 경로 | 🔴 음악 라이브러리 배포 차단. 🔺 **경로가 리드타임과 규모를 함께 바꾼다** | 사업 · 법무 | **Phase 1 전** — 🔺 **Q17 실측 후가 유리** |
...
| **Q19** | 🔴 **후보 정렬 기준을 사용자가 고를 수 있어야 하는가** | REQ-FUNC-004 · §2.4.1 — *"잘 나온 것"* 과 *"처음 된 것"* 은 다른 순서다 | 제품 | Phase 1 실측 후 |
| **Q20** | 🔴 **지표에 잡히지 않는 이탈을 어떻게 관측할 것인가** | §2.4.2 — 완주율은 *"시작한 작업"* 만 센다 | 제품 · 데이터 | Phase 2 전 |
| **Q21** | **REQ-FUNC-020(타임라인)을 MVP로 올릴 것인가** | §2.4.3 — 세 이용자의 **최고 감정 지점**이자 O7의 체감 경로 | 제품 | MVP 범위 확정 전 |
| **Q22** | 🆕 **무료 등급 쿼터 값** — 월 편수 · 원본 분 · 재발행 횟수 · 보관 기간 | 🔴 **ADR-8의 이행 판정 전제.** 값 없이는 유료 등급을 설계할 수 없고, REQ-NF-013의 *"무제한 업로드를 열지 않는다"* 도 집행 불가다 | 사업 · 백엔드 | **Q3 해소 직후** |
| **Q23** | 🆕 **음악 곡 풀을 무료/유료로 나눌 것인가** | REQ-FUNC-007 · 🔺 무료 풀이 부실하면 **O2(완성 전환율)가 깨진다** — REF-01이 F18a를 MVP에 넣은 논거가 *"음악이 없으면 다른 앱으로 내보낸다"* 였다. **T5 선행** | 사업 · 제품 | **T5 해소 후** |
| **Q24** | 🆕 **학습용 원본 보관 동의율** — 무료 등급 opt-in 수락률 | 🔴 **ADR-9 ②의 성립 전제.** 동의율이 낮으면 라벨(①)이 남아도 학습이 시작되지 않는다. **Baseline: 미측정 · Target: TBD** | 제품 · 법무 | **`FACE_CONSENT` 승인 후 베타** |
...
> 근거: REF-05 §9.6.13 Modes and states

## 7.1 기록의 공개 범위 상태 전이

> **그림 7-1. 공개 범위 상태 전이도 (State Diagram)** — 기록 하나가 가질 수 있는 상태와 그 전이를 일으키는 사건이다.
...
> 🔴 **불변 조건 — 어떤 전이에서도 기록 자체는 삭제되지 않는다.** 공개 범위 변경은 **노출 범위만** 바꾼다. 삭제로 가는 화살표는 **사용자가 명시적으로 지울 때 하나뿐**이다.

## 7.2 편집 파이프라인 단계

> **그림 7-2. 편집 파이프라인과 실패 분기 (Flow Chart)** — 영상 한 편이 만들어지는 6단계와, 각 단계에서 실패했을 때 어디로 빠지는지다.
...
**주황(2단계)은 Gate A 판정 지점**, **초록(4단계)은 차별점인 구도 보정**, **노랑(기록 저장)은 Gate B의 전제**다. 🔴 **저장이 6단계보다 앞에 있는 것이 이 그림의 핵심이다.**

| 단계 | 화면 | 담당 요구사항 | 다음 단계 진입 조건 |
| --- | --- | --- | --- |
| 1 · 원본 고르기 | 갤러리 격자 | REQ-FUNC-001 | 코덱 사전 검증 통과 (SC-1.F1) |
| 2 · 내 장면 찾기 | 탐지 진행 | REQ-FUNC-002 · 003 · 027 | 등장 구간 1개 이상 (미검출 시 SC-1.F2) |
| 3 · 하이라이트 선택 | 후보 목록 | REQ-FUNC-004 · 005 | `selection_confirmed` 발생 |
| 4 · 구도 보정 | 결과 확인 | REQ-FUNC-006 | 사용자 확인 (불만족 시 SC-3.F2) |
...
현재 범위는 **결정론적 인물 추적**과 **단일 카테고리(농구·구기)** 에 초점을 둔다. 다음은 향후 버전에서 계획된다.

## 8.1 선택 데이터 학습 순환 (REQ-FUNC-023)

사용자 선택 데이터를 축적해 탐지 후보의 정확도를 높이는 순환. **화면 복제로 따라올 수 없는 방어선**이나 **학습 데이터가 사용자 없이는 생기지 않으므로 순서상 후순위**다. 선택 이벤트(`selection_confirmed`)가 학습 데이터원이 된다.

## 8.2 구도 고도화 및 기록 심화 (REQ-FUNC-018 · 020 · 021)

촬영 노하우 기반 **구도 규칙과 앵글 연출**로 결과물 완성도 향상 · 날짜별 **타임라인**으로 성장을 눈으로 확인하는 뷰 · 온보딩 **취향 설문**을 추천 피드에 반영.

> 🔺 **REQ-FUNC-020(타임라인)은 §2.4.3에 따라 MVP 재검토 대상이다.** 세 이용자의 최고 감정 지점이 여기다.

## 8.3 저장공간 회수 및 노출 자리 (REQ-FUNC-019 · 022)

결과물 확보 후 **원본 삭제를 안내**해 저장공간 압박 해소 · **카테고리별 랭킹**으로 노출 자리 제공 — **도입 여부 자체는 미정**.

## 8.4 팀·다중 인물 확장 (REQ-FUNC-024)

한 원본을 여러 사용자가 사용하는 팀 공유 편집. 기술적으로는 **다중 인물 동시 추적**이 핵심이며, **처리 물량이 인원수에 비례해 원가에 직접 영향**을 준다. 팀 계정 구조 · 원본 공유 권한 · 얼굴 정보 동의 · 원가 부담 주체 **넷이 전부 미정**이다.

## 8.5 카테고리 확장

첫 검증은 **농구 한 카테고리**로 좁혀 시작하며 이후 구기 종목 전반으로 확장한다. **확장 시점은 Gate A 통과에 연동**되며 1년차 목표 달성 경로와 함께 재판정한다.

---

# 9. 다이어그램 색인

**본 문서에 포함된 설계 다이어그램 17종이다.** 배경지식 없이 처음 읽는다면 **그림 4-1 → 3-1 → 7-2** 순서로 보면 제품의 전체 구조가 가장 빨리 잡힌다.

| 그림 | 종류 | 위치 | 무엇을 답하는가 |
| :--: | --- | :--: | --- |
| **2-1** | Flow Chart | §2.4 | **사용자는 어디서 포기하고, 무엇이 그것을 막는가** |
| **3-1** | Component Diagram | §3.3 | 어떤 서비스가 어떤 순서로 연결되는가 |
| **4-1** | Use Case Diagram | §4 | **누가 이 시스템으로 무엇을 하는가** |
| 4-2 | Flow Chart | §4.3 | 요구사항 의존과 구현 규모는 어떻게 나뉘는가 |
| **6-1** | ERD | §6.2 | 데이터 구조와 참조 관계는 어떻게 되는가 |
| 6-2 | Class Diagram | §6.2 | 분류값(열거형)은 어디에 붙는가 |
| 6-3 ~ 6-7 | Sequence Diagram | §6.4 | 업로드 재개 · 탐지 계측 · 렌더 실패 · 공개범위 강제 · 그룹 회수 |
| 6-8 | Flow Chart | §6.5 | 어떤 순서로 검증하고 실패하면 무엇을 하는가 |
| **7-1** | State Diagram | §7.1 | **기록의 공개 상태는 어떻게 바뀌는가** |
| **7-2** | Flow Chart | §7.2 | **영상 한 편이 만들어지는 6단계와 실패 경로는** |
| 3-2 ~ 3-5 | Sequence Diagram | §3.5 | 업로드·탐지 / 선택·완성 / 저장·공개범위 / 피드·강제 |

> **공통 표기 규약** — **주황**은 Gate A(추적 정확도) 관련, **노랑**은 Gate B(기록 공간) 관련, **점선 테두리**는 실패·중단 경로, **원통**은 외부 시스템이다.
