...
> | 측정 시점 | 무엇을 재는가 | 문제 |
> | --- | --- | --- |
> | **제외 전** | 모델의 순수 탐지 능력 | 사용자가 실제로 받는 결과와 다르다 |
> | **제외 후** | 사용자가 실제로 받는 후보 | **제외 임계를 높이면 O9가 자동으로 하락**한다 |
>
> **[PROPOSED] 둘 다 잰다.** Gate A 판정은 **제외 전 탐지율**로 하고(모델 성능 판정이므로), **제외 후 탐지율**을 별도 지표로 함께 집계한다. 두 값의 차이가 곧 **제외로 잃은 양**이며, 이 차이가 커지면 임계를 재조정한다. **한 값만 재면 임계 조정으로 지표를 만들어낼 수 있다.**
...
| **Q10-b** | **영상 내 미성년자 판별 기준을 무엇으로 할 것인가** | 🔴 **공개 발행 배포 차단** · 클럽 파일럿의 전제 | 법무 · 제품 | **클럽 파일럿 착수 전** |
