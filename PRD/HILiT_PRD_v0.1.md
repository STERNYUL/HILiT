...
| **ADR-2** | 처리 방식 | ✅ **결정 — 서버 GPU** [PROPOSED] |
...
### ✅ ADR-2 · 처리 방식은 **서버 GPU**로 한다

**맥락** — §5(NFR)·§6(데이터·인터페이스)이 이미 비동기 큐·편당 GPU 원가·탐지 p95 8분을 전제로 쓰여 있다. **결정이 문서 전체에 이미 반영돼 있는데 ADR만 미정으로 남아 있는 것은 내부 모순이다.** 이 항목은 새 결정이라기보다 **암묵적 전제의 명문화**다.

**결정** — 서버 GPU 처리. **온디바이스는 MVP 범위에서 제외**한다.

**근거**
1. 40분·4GB 원본을 단말에서 처리하면 **발열·배터리·소요 시간이 사용자 경험을 지배**한다. Core Job의 "이미 찍어둔 긴 영상"이 곧 이 부담의 크기다
2. F2b는 **Gate A 판정 대상**이라 모델을 자주 교체·재학습해야 한다. 온디바이스는 모델 갱신이 **앱 배포 주기에 묶인다** — 수렴형 작업(§4.4 L)과 배포 주기가 충돌한다
3. 경쟁사가 하드웨어를 쓰는 이유가 처리 부담이다. **우리는 "하드웨어를 안 써도 되는 종목"을 고른 것이지, "처리를 단말에 떠넘긴다"를 고른 것이 아니다**

**감수하는 것 (Consequences)**
- **원본이 서버로 올라간다.** 온디바이스였다면 없었을 프라이버시 부담이 생긴다 — **제3자 얼굴(G4)·삭제 요청(§5.4)의 무게가 커진다.** Q4를 배포 게이트로 둔 이유가 여기서도 확인된다
- **GPU 원가가 사용량에 비례한다**(R3). 성장이 곧 비용이다 → ADR-3이 이 결과를 완화한다

**재검토 조건** — 편당 GPU 원가가 **Q3에서 정할 상한을 넘으면** 하이브리드(1차 탐지만 온디바이스)를 재검토한다.

---

### ✅ ADR-3 · 후보는 **저해상도로 만들고, 고른 것만 고화질**로 올린다

**맥락** — 40분 원본에서 후보 **약 30개**를 만든다. 그런데 사용자가 실제로 쓰는 것은 보통 소수다. 30개를 전부 고화질로 처리하면 **쓰이지 않을 대부분의 처리비가 버려진다.**

**결정** — 탐지·후보 생성은 **저해상도**로 수행하고, **사용자가 선택한 구간만 원본 해상도로** F5a 크롭·리프레이밍·렌더한다.
...
### ⏸ 보류 3건 — 지금 정하면 안 되는 이유

| # | 결정 | 왜 지금 정하지 않는가 |
| --- | --- | --- |
| **ADR-1** | 추적 모델 조달<br>*(공개 모델 파인튜닝 / 자체 학습 / 상용 API)* | **Gate A 결과에 종속된다.** 공개 모델로 O9 85%가 나오면 자체 학습은 불필요하고, 안 나오면 상용 API도 같은 조건에서 재평가해야 한다. **순서를 어기고 지금 정하면 어느 쪽이든 근거 없는 선택이 된다** — §12.1의 순서 원칙과 같다 |
| **ADR-4** | 공개 범위 기본값<br>*(나만 보기 / 그룹 / 전체공개)* | **논리는 "나만 보기"를 가리킨다** — P3("공개할 자신이 없으면 기록도 남지 않는다")·D4("기록이 먼저고 공개가 선택")·O5(비공개 30%)가 모두 그쪽이다. **그런데 그러면 F13 피드에 볼 것이 없어져 AF-8이 상시 발동하고 O7 재방문 루프가 첫 진입에서 끊긴다.** 한쪽을 고르면 다른 쪽이 깨지는 **양방향 리스크**라 논리로 못 푼다 → **E3에서 판정** |
