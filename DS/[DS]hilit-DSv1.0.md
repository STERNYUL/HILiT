...
**상위 문서:** `[SRS]hilit-SRSv1.5.md` (요구사항 45건 · 시나리오 42건)
...
| 엔티티 속성 · 타입 · 키 · 제약 · 인덱스 · 보존 정책 | 실험 설계 · 게이트 판정 (SRS §6.4) |
...
| ID | 문서 |
| --- | --- |
| **DREF-01** | `[SRS]hilit-SRSv1.5.md` — 요구사항 원천 |
...
> 감사 **P1-6** 해소. SRS §2.2의 이용자 클래스 UC-01~06과 요구사항을 연결한다.
...
flowchart LR
    ...
    subgraph S["시스템 경계 — HILiT"]
        C1["UCS-1 원본에서 내 장면 찾기"]
        C2["UCS-2 장면 고르고 완성하기"]
        C3["UCS-3 공개 없이 기록 남기기"]
        C4["UCS-4 그룹에만 열기"]
        C5["UCS-5 피드 보고 반응하기"]
        C6["UCS-6 원본 정리하기"]
        C7["UCS-7 동의 관리"]
...
| Use Case | 행위자 | 대응 요구사항 | Story |
| --- | --- | --- | --- |
| **UCS-1** 원본에서 내 장면 찾기 | UC-01 · 02 · 05 | REQ-FUNC-001 · 002 · 003 · 027 | US-1 · US-4 · US-6 |
| **UCS-2** 장면 고르고 완성하기 | UC-01 · 04 · 05 | REQ-FUNC-004~008 · REQ-NF-018 | US-1 · US-3 · US-6 |
| **UCS-3** 공개 없이 기록 남기기 | UC-01 · 03 · 06 | REQ-FUNC-009 · 010 | US-2 |
| **UCS-4** 그룹에만 열기 | UC-02 · 06 | REQ-FUNC-013 · REQ-NF-009 | US-4 |
| **UCS-5** 피드 보고 반응하기 | UC-01 · 06 | REQ-FUNC-011 · 012 · 014~017 | US-5 |
| **UCS-6** 원본 정리하기 | UC-03 | REQ-FUNC-019 · SC-4.F2 | US-2 |
| **UCS-7** 동의 관리 | 법정대리인 | REQ-NF-016 · 017 | — |
| **UCS-8** 신고 처리 | 운영자 | REQ-FUNC-016 | US-5 |

> **UCS-7·8의 행위자는 SRS §2.2에 없다.** 법정대리인과 운영자는 이용자 클래스가 아니지만 **시스템과 상호작용하는 행위자**다. 🔺 SRS §2.2에 행위자 구분 추가 필요(§9-1).
...
| **A-04** | `DELETE /videos/{id}` | — | `204` | `404` · `409` 처리 중 | ➖ | REQ-FUNC-019 · REQ-NF-022 |
...
> **논리/물리 삭제 분리 근거** — REQ-NF-022는 *"원본과 모든 파생물을 전량 삭제"* 를 요구한다. 논리 삭제는 이를 만족하지 않는다.
...
**시퀀스 다이어그램은 SRS §3.5·§6.3에 9개가 있다.** 여기서는 SRS가 다루지 않은 **상태 전이**만 설계한다.
...
| --- | --- | --- |
| §2.1 Use Case 모델 (UCS-1~8) | REQ-FUNC 전체 · REQ-NF-016·017 | **P1-6** |
...
| # | 내용 | 근거 | 제안 |
| :--: | --- | --- | --- |
| **9-1** | **행위자 구분 부재** — 법정대리인·운영자가 §2.2 이용자 클래스에 없다 | UCS-7·8의 행위자 | §2.2에 **행위자 절 분리** 또는 §2.1 이해관계자와 연결 |
